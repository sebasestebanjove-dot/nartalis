...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { letter, page } = await params;
  const p = parseInt(page, 10);
  const l = letter.toUpperCase();
  if (!/^[a-z]$/i.test(letter) || isNaN(p) || p < 2) return { title: 'Página no encontrada' };
  return catalogMetadata(
    `Medicamentos con ${l} — Página ${p} | Nartalis`,
    `Consulta la página ${p} de medicamentos que comienzan por ${l} y accede a sus prospectos e información oficial de la AEMPS.`,
    `https://nartalis.com/medicamentos/${letter}/${page}`,
  );
}
...
export default async function LetterPaginatedPage({ params }: Props) {
  const { letter, page } = await params;
  if (!/^[a-z]$/i.test(letter)) notFound();

  const p = parseInt(page, 10);
  if (isNaN(p) || p < 2) notFound();
  if (!Number.isInteger(p)) notFound();
  ...
  const drugs = await getDrugsByLetter(l, p);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: l, item: `https://nartalis.com/medicamentos/${letter}` },
      { '@type': 'ListItem', position: 4, name: `Página ${p}` },
    ],
  };
  ...
  return (
    <div style={S.page}>
      ...
      <nav aria-label="Breadcrumb" style={S.breadcrumb}>
        ...
        <span style={S.breadcrumbSep}>/</span>
        <Link href={`/medicamentos/${letter}`} style={S.breadcrumbLink}>{l}</Link>
        <span style={S.breadcrumbSep}>/</span>
        <span style={{ color: '#64748B' }}>Página {p}</span>
      </nav>

      <h1 style={S.h1}>Medicamentos con {l} — Página {p}</h1>
      <p style={S.subtitle}>
        Página {p} de {totalPages}. Total: {total} medicamento{total !== 1 ? 's' : ''} que comienzan por {l}. Accede a sus prospectos y datos oficiales de la AEMPS.
      </p>
      ...
      <PageNav letter={letter} page={p} totalPages={totalPages} />
